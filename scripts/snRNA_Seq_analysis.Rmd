...
The boxplot to the left suggest tha there's a significant proportion of cells with mit gene expression, although is difficult to say. On the right plot, we are looking at the distribution of mitochondrial gene expression, and the vertical bars mark the 0.5% and 1% content. The majority of cells contain < 0.5%. As seen in other papers mit content below 1% is found in other datasets as seen here:  <https://www.nature.com/articles/s41591-020-0844-1.pdf>
...
## Analysis samples separately {.tabset .tabset-fade}
...
sce <- JackStraw( x, num.replicate=50, dims=50 )
sce <- ScoreJackStraw( sce, dims=1:50)
#print(JackStrawPlot(sce, dims = 1:50))
#print( ElbowPlot(sce) )

})

sceL2 <- sapply( sceL2, function (x) {
sce <- FindNeighbors(x, dims = 1:40)
sce <- FindClusters(sce, resolution = 0.5)
sce <- RunUMAP( sce, dims = 1:40)
})

 saveRDS(sceL2, paste0(resultsDir, "sceL2.rds"))

```


```{r render_umaps_per_sample,results='asis',echo=FALSE,fig.wigth=12,fig.height=6}

for( i in names( sceL2 ) ) {
    cat( "### ", i, " \n" )
     p1 <- DimPlot(sceL2[[ i ]], reduction ="umap")
    p2 <- DimPlot(sceL2[[ i ]], reduction ="umap", group.by="Phase" )
   grid.draw( p1 | p2 )
    cat( " \n\n\n" )
}
...
### Doublet Finder {.tab, .tab-fade}

```{r, doublet_finder, echo=FALSE, message=FALSE, include=FALSE, warning=FALSE}

 library(DoubletFinder)

sceL3 <- sapply( sceL2, function (sce) {
  ## pK Identification (ground-truth) 
 sweep.res.sce <- paramSweep_v3(sce, PCs = 1:10, sct = FALSE)
 gt.calls <- sce@meta.data[rownames(sweep.res.sce[[1]]), "GT"]
 sweep.res.sce <- summarizeSweep(sweep.res.sce, GT = FALSE)
 bcmvn_sce <- find.pK(sweep.res.sce)

#write.table(bcmvn_sce, file="./DoubletFinder/doubletFinder_HEW569A17.txt", sep="\t")
bcmvn_sce

# #create plot
# plot(x=as.numeric(bcmvn_sce$pK), y=bcmvn_sce$BCmetric, pch=16, col="#41b6c4", cex=0.7,
# xaxt = "n", xlab="pK", ylab="BCmvn", main="DoubletFinder_HEW569A17")
# lines(x=bcmvn_sce$pK, y=bcmvn_sce$BCmetric, col="#41b6c4")
# axis(1, 1:31, bcmvn_sce$pK)
# abline(v=rownames(bcmvn_sce[bcmvn_sce$BCmetric == max(bcmvn_sce$BCmetric),]) , col="red", lty=2)

## Homotypic Doublet Proportion Estimate

annotations <- sce@meta.data$seurat_clusters
homotypic.prop <- modelHomotypic(annotations)
nExp_poi <- round(0.075*length(rownames(sce@meta.data)))  ## Assuming 7.5% doublet formation rate - tailor for your dataset
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))

##we use the maximum value of pK from the first plot to then call the doublets)
sce <- doubletFinder_v3(sce, PCs = 1:10, pN = 0.25, pK = 0.03, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)

# png("./DoubletFinder/HEW569A17_doublet_umap_plot.png");
# DimPlot( sce, reduction = "umap", group.by="DF.classifications_0.25_0.03_159") + ggtitle("DoubletFinder");
# dev.off();
sce
})

 saveRDS(sceL3, paste0(resultsDir, "sceL3.rds"))

```



```{r, doublet_finder_plots, results='asis',echo=FALSE,fig.wigth=12,fig.height=6 , eval=FALSE}


for( i in names( sceL3 ) ) {
    cat( "#### ", i, " \n" )
  group <-  names(sceL3[[i]]@meta.data)[ grep("DF.classifications_", names(sceL3[[i]]@meta.data))]
  grid.draw( DimPlot( sce, reduction = "umap", group.by= group + ggtitle("DoubletFinder") )
    cat( " \n\n\n" )
}
...
RidgePlot(sceL3[[1]], features="nFeature_RNA", assay="RNA", group.by="Doublets")
RidgePlot(sceL3[[1]], features="nCount_RNA", assay="RNA", group.by="Doublets")

```


At this particular resolution (0.6), there are few clusters that have very low number of genes detected, not sure if this is biology or that's a signal of something going wrong. 
...
The tree above, shows how clusters change as we increase the parameter called  "resolution". Below there are heatmaps generated with the top10 gene markers for each resolution, so you can see how things change. Additionally, you are given a document "Cluster_markers.xlsx" that should give cell markers for each cluster at each resolution. That should help you explore your data and determine which cluster resolution best explain the underlying biology. 
...
I'm  using a list of markers  provided my Micaela in the past for a previous project, so I'm hoping they will be of some use.
